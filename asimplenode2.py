...
def on_connect(client, userdata, flags, rc):
	...
	if rc > 0:
		...
		client.subscribe("testing", qos=0)
		client.subscribe("external/testing", qos=0)
		client.subscribe("global/testing", qos=0)
...
def on_message(client, userdata, msg):
	...
	elif msg.topic.startswith("external"):
...
def producing_data(snum):
	while True:
		...
		if random.random() > 0.0:
			...
			client.publish("testing",the_msg_str)
			sys.stdout.write("SENSOR "+str(snum)+": produced INTERNAL message %s\n" % the_msg_str)

		time.sleep(10)# sleep for x seconds before next call
