...
LOCAL_EID = 'themqgat' # Endpoint ID for the IBR-DTN protocol

# Debugging related stuff
DEBUG_MSG_ON = False
logging.basicConfig(level=logging.DEBUG, format='[%(levelname)s] [%(threadName)-10s%(message)s',)

ite_stats = []   # list containing the processing time for internal to external messages
eti_stats = []   # list containing the processing time for external to internal messages
...
# send to the MQTT local net
	...
	inb_link = ddtalker.ddtalker(dd_host, dd_port)

	if DEBUG_MSG_ON: logging.debug(":mqttproxycore:handle_input_from_outside]")
	inb_link.recv_set_endpoint(LOCAL_EID)
	while True:
		in_msg = inb_link.recv_1_bundle()
#		time1 = time.time()
...
#		time2 = time.time()
#		eti_stats.append((time2-time1)*1000) # elapsed time in msec
#		sys.stdout.write("mqttproxy:eti_stats: ")
#		print eti_stats
...
if __name__ == '__main__':
	...
	OUTB_LINK = ddtalker.ddtalker(DD_HOST, DD_PORT)
